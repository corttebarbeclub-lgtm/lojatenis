...
async function generateBatchSQL() {
  console.log(`Gerando SQL em lote para ${SNEAKERS.length} modelos...`);
...
begin
  ...
  -- Limpa catálogo anterior para recriar os 50 modelos de forma limpa e consistente
...
`;
  ...
  for (let i = 0; i < SNEAKERS.length; i++) {
    ...
    const cName = s.category.replace(/'/g, "''");
    const pName = s.name.replace(/'/g, "''");
    const pDesc = s.description.replace(/'/g, "''");
    const pImg = s.image.replace(/'/g, "''");

    sql += `
  -- [${i + 1}/50] ${s.name}
  insert into brands (tenant_id, name) values (v_tenant_id, '${bName}') on conflict (tenant_id, name) do nothing;
  select id into v_b_id from brands where tenant_id = v_tenant_id and name = '${bName}' limit 1;

  insert into categories (tenant_id, name) values (v_tenant_id, '${cName}') on conflict (tenant_id, name) do nothing;
  select id into v_c_id from categories where tenant_id = v_tenant_id and name = '${cName}' limit 1;

  insert into products (tenant_id, name, brand_id, category_id, description, gender, is_active)
  values (v_tenant_id, '${pName}', v_b_id, v_c_id, '${pDesc}', '${s.gender}', true)
  on conflict do nothing;
  select id into v_p_id from products where tenant_id = v_tenant_id and name = '${pName}' limit 1;

  insert into product_images (tenant_id, product_id, storage_path, url, position, is_primary)
  values (v_tenant_id, v_p_id, 'products/' || v_p_id || '/primary.jpg', '${pImg}', 0, true)
  on conflict do nothing;
`;

    for (let cIdx = 0; cIdx < s.colors.length; cIdx++) {
      const color = s.colors[cIdx];
      const colorClean = color.replace(/'/g, "''");

      for (const size of s.sizes) {
        const sku = `${s.brand.slice(0, 3).toUpperCase()}-M${i + 1}-C${cIdx + 1}-${size}`;
        const stockQty = Math.floor(Math.random() * 15) + 10;

        sql += `
  insert into product_variants (tenant_id, product_id, sku, color, size, price_cents, wholesale_price_cents, wholesale_min_qty, is_active)
  values (v_tenant_id, v_p_id, '${sku}', '${colorClean}', '${size}', ${s.retailPrice}, ${s.wholesalePrice}, ${s.wholesaleMinQty}, true)
  ...
  if v_v_id is null then
    select id into v_v_id from product_variants where product_id = v_p_id and color = '${colorClean}' and size = '${size}' limit 1;
  end if;

  insert into inventory (tenant_id, variant_id, quantity, min_quantity)
  values (v_tenant_id, v_v_id, ${stockQty}, 2)
  on conflict (variant_id) do update set quantity = ${stockQty};
`;
      }
    }
  }
...
`;
  ...
  console.log(`Enviando transação atômica para o Supabase... (tamanho do script: ${(sql.length / 1024).toFixed(1)} KB)`);
  const result = await runSQL(sql);
  console.log(`✅ Sucesso! Resposta:`, result);
  console.log(`\n🎉 Todos os 50 modelos, variações e estoques únicos foram gravados com sucesso!`);
}

generateBatchSQL().catch((err) => {
