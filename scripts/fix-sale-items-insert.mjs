...
async function fixSaleItems() {
  ...
  `);
  console.log('Colunas de sale_items:', cols.map((c) => c.column_name));

  const pCols = await runSQL(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_name = 'payments';
  `);
  console.log('Colunas de payments:', pCols.map((c) => c.column_name));
  ...
  const sql = `
    ...
    begin
      ...
      -- 3. Inserir itens da venda (apenas colunas existentes)
      for v_item in select * from jsonb_array_elements(p_items)
      loop
        v_variant_id := (v_item->>'variant_id')::uuid;
        v_qty := (v_item->>'quantity')::int;
  ...
  await runSQL(sql);
  console.log('✅ RPC create_storefront_order corrigida perfeitamente!');
}

fixSaleItems().catch(console.error);
